...
export async function updateTask(taskId: string, task: ITask): Promise<ITask | AxiosError | null> {
    try {
        const result: AxiosResponse<ITask> = await httpAxios.put(`/api/task/${taskId}`, task);
        return result.data;
    } catch (error) {
        if (error instanceof AxiosError) {
            return error;
